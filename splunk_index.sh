...
hostnamectl set-hostname splunk-indexer
#deploy splunk and build file structure
/opt/splunk/bin/./splunk start --accept-license --no-prompt --answer-yes
#add a defaut splunk user so that you can run splunk not as root
...
sudo -u splunk /opt/splunk/bin/./splunk set deploy-poll 10.0.0.1:8089 -auth admin:overlord
