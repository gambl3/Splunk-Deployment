...
###grubbs comment: maybe use "firewall-cmd --reload" for consistency?
systemctl restart firewalld
hostnamectl set-hostname splunk-sa
...
# backup splunk generic apps and replace with an app package
mv /opt/splunk/etc/apps /opt/splunk/etc/apps.BAK
cp -r /data/splunk/apps /opt/splunk/etc/

#cp /tmp/local/server.conf /opt/splunk/etc/system/local/
#cp -r /data/splunk/deployment-apps/* /opt/splunk/etc/apps/
...
# sudo -u splunk /opt/splunk/bin/./splunk set deploy-poll 10.1.51.57:8089 -auth admin:overlord
